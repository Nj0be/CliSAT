...
std::uint64_t randll() {
    return (static_cast<std::uint64_t>(std::rand()) << 32) | std::rand();
}
...
void bitscan_benchmark1() {
    ...
    std::vector<std::uint64_t> numbers(50000000);
...
void popcount_benchmark() {
    ...
    std::vector<std::uint64_t> numbers(500000000);
    std::ranges::generate(numbers, randll);
...
void bitwise_operations_variants_benchmark() {
    ...
    auto and_if = [](custom_bitset& result, const custom_bitset& other) {
        for (std::uint64_t i = 0; i < result.bits.size(); ++i) {
...
void operator_variants_benchmark() {
    ...
    auto verify_results = [](const custom_bitset& a, const custom_bitset& b, const custom_bitset& c) {
        ...
        for (std::uint64_t i = 0; i < a.bits.size(); i++) {
    ...
    std::uint64_t len1 = 50000000;
    std::uint64_t len2 = 30000000;  // Different size to test size handling
    std::uint64_t iterations = 100;
    std::uint64_t acc = 0;
