...
inline bool BB_ReCol(const custom_graph& g, const std::uint64_t v, std::vector<custom_bitset>& C_sets, const std::int64_t k_min) {
    ...
    for (std::int64_t k1 = 0; k1 < k_min; ++k1) {
        ...
        if (inters.next(w) == custom_bitset::npos) {  // |set| = 1 -> double swap
            for (std::int64_t k2 = k1+1; k2 < k_min; ++k2) {
                inters = C_sets[k2];
...
inline void BB_ColorR(const custom_graph& g, custom_bitset Ubb, std::vector<std::uint64_t>& Ul, std::vector<std::uint64_t>& C, const std::int64_t k_min=0) {
    static std::vector C_sets(g.size(), custom_bitset(g.size()));

    for (std::int64_t k = 0; Ubb.any();) {
        C_sets[k] = Ubb;
...
inline void BBMCR(const custom_graph& g, custom_bitset& Ubb, std::vector<std::vector<std::uint64_t>>& Ul, std::vector<std::vector<std::uint64_t>>& C, custom_bitset& S, custom_bitset& S_max, const std::uint64_t depth=0) {
