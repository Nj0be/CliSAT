...
public:
    ...
    explicit custom_graph(std::string filename);

    void add_edge(uint64_t u, uint64_t v);
    void remove_edge(uint64_t u, uint64_t v);

    uint64_t size() const { return graph.size(); }
    uint64_t get_n_edges() const { return n_edges; }
};

inline custom_graph::custom_graph(const uint64_t size) {
    graph.reserve(size);
    for (uint64_t i = 0; i < size; ++i) {
        graph.push_back(custom_bitset(size));
    }
}

// TODO: change assert
inline custom_graph::custom_graph(std::string filename) {
    ...
    while (std::getline(inf, strInput)) {
        if (strInput[0] == 'p') {
            ...
            for (uint64_t i = 0; i < nodes; ++i) {
                graph.push_back(custom_bitset(nodes));
            }
        }
